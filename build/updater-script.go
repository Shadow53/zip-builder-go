    ...
    for i, ver := range lib.Versions {
        ...
        if item[ver].Base != "" {
            if item[ver].Base != ver && i < len(lib.Versions) - 1 {
                if multVersionTest != "" { multVersionTest = multVersionTest + " || " }
            ...
            if item[ver].Base == ver {
                if multVersionTest == "" { multVersionTest = testVersion }
                if !item[ver].HasArchSpecificInfo {
                    ...
                    for _, arch := range lib.Arches {
                        if item[ver].Arch[arch].FileName != "" {
                            ...
                            if arch == "arm" { archBuff.WriteString("armeabi") } else { archBuff.WriteString(arch) }
