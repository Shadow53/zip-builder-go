                ...
                for _, a := range lib.Arches {
                    if !app.AndroidVersion[ver].HasArchSpecificInfo || arch == a {
                        if (a == "arm" && strings.Index(fileName, "armeabi") > -1) || (a != "arm" && strings.Index(fileName, a) > -1) {
                            ...
                            for i := sort.SearchStrings(lib.Versions, ver);
                                i < len(lib.Versions) && app.AndroidVersion[lib.Versions[i]].Base == ver;
                                i++ {
                                    v := lib.Versions[i]
            ...
            if apps[app].AndroidVersion[ver].Base == ver {
                for _, arch := range lib.Arches {
                    if apps[app].AndroidVersion[ver].Arch[arch].Url != "" {
                        ...
                        if apps[app].AndroidVersion[ver].HasArchSpecificInfo { filename = filename + "-" + arch }
                    ...
                    if !apps[app].AndroidVersion[ver].HasArchSpecificInfo { break }
                }
            } else {
            ...
            if files[file][ver].Base == ver {
                for _, arch := range lib.Arches {
                    if files[file][ver].Arch[arch].Url != "" {
                        ...
                        if files[file][ver].HasArchSpecificInfo { filename = filename + "." + arch }
                    ...
                    if !files[file][ver].HasArchSpecificInfo { break }
                }
            }
        }
	}
